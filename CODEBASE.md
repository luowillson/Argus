...
├── Makefile                    ← common local/dev commands
├── docker-compose.yml          ← Postgres (pgvector) + Redis
├── openreview_reviews.py       ← standalone OpenReview scoring CLI
├── scoring/                    ← local scoring package used by standalone tooling
├── score_scales.json           ← local venue score-scale database
├── paper_scores.json           ← local score cache generated by standalone tooling
├── data/                       ← JSONL outputs from bulk parsing scripts
├── reviews2.md                 ← generated sample review export
├── scripts/                    ← standalone/bulk OpenReview parsing scripts
├── Designs/                    ← static JSX mockups (visual reference only)
├── api/                        ← FastAPI backend
│   ├── .env                    ← environment variables (never commit real keys)
│   ├── .env.example            ← local defaults and required settings
│   ├── shared-db.env.example   ← template for shared team Postgres
│   ├── pyproject.toml
│   ├── alembic.ini
...
Team development uses one shared Postgres database when possible. Do not try to
sync developer Docker volumes or database dumps as the normal workflow. Point
`api/.env` at the hosted pgvector Postgres instance, then run Alembic migrations
from `api/`:
...
Keep Redis local unless the team intentionally provisions a shared queue. Redis
only coordinates Celery work; the reusable product data lives in Postgres. Each
developer should set a unique `DEMO_USER_ID` / `DEMO_USER_EMAIL` against the
shared database so `saved_papers` does not collapse into one shared demo user.

Use `api/shared-db.env.example` for the shared Postgres template and
`api/.env.example` for the full local env template.
...
GET /papers/{id}  →  paper not in DB
  ...
  → ingest_paper_task.delay(forum_id)      [Celery task]
      ...
      → analyze_paper()                    [LLM → ai_insights, best-effort]
      → embed_paper_task.delay(paper_id)   [queued after successful ingest task]
...
**Next/React versions** — `web/package.json` currently pins Next `16.2.4`, React `19.2.4`, and TypeScript 5.

**Tailwind v4** — color and font tokens are in `web/src/app/globals.css` as CSS `@theme` variables, not `tailwind.config.ts`. Custom colors include `paper`, `ink`, `burgundy`, `cream`, `rule`, `rule-soft`, `muted`, `muted-2`, `prose`, `prose-soft`, `accept`, `borderline`, `surface`, `bg-warm`, and `border-c`.

**Next.js App Router** — `params` and `searchParams` are Promises and must be awaited:
```tsx
...
Auth is stubbed. `api/app/deps.py` `get_current_user()` returns `CurrentUser(id=settings.demo_user_id, email=settings.demo_user_email)`, defaulting to `demo-user` / `demo@veros.local`. Swap this function for a JWT verifier when real auth is needed — all routers use `CurrentUserDep` so no other changes are needed.

---

## Key conventions

- **Run API `uv` commands from `api/`** — the FastAPI project and importable `app` package live there.
...
- **LLM max_output_tokens=4000** — necessary for Gemini-compatible JSON generation; 1500 caused mid-string JSON truncation.
...
For shared development, `DATABASE_URL` is overridden in `api/.env` with the
hosted team Postgres URL. The URL should point to a database where `vector` and
`pg_trgm` can be created by Alembic.
