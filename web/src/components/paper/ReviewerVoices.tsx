    ...
    <section className="mt-9 border-t-[1.5px] border-ink pt-5">
      ...
      {paper.reviewers.length === 0 ? (
        <p className="py-8 text-center font-sans text-[13px] text-muted">
          Reviews not visible — this venue may require authentication or has not
          published official reviews publicly.
        </p>
      ) : (
        <ul>
          {paper.reviewers.map((rv) => (
            <li key={rv.handle} className="border-b border-rule-soft py-3.5">
